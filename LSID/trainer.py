...
def to_numpy(t):
    return t.detach().cpu().numpy() if t.requires_grad is True else t.cpu().numpy()
...
class Trainer(object):
    ...
    def __init__(self, cmd, cuda, model, criterion, optimizer,
                 ...
                 checkpoint_dir=None, result_dir=None, use_camera_wb=False, print_freq=1, onnx_session=None):
        """
        :param cuda:
        :param model:
        :param optimizer:
    ...
    def validate(self):
        ...
        for batch_idx, (raws, imgs, targets, img_files, img_exposures, lbl_exposures, ratios) in tqdm.tqdm(
            ...
            if self.cuda:
                ...
                raws, targets = raws.cuda(), targets.cuda()
...
class Validator(Trainer):
    ...
    def __init__(self, cmd, cuda, model, criterion, val_loader, log_file, result_dir=None, use_camera_wb=False,
                 print_freq=1, onnx_session=None):
        super(Validator, self).__init__(cmd, cuda=cuda, model=model, onnx_session=onnx_session, criterion=criterion,
